...
router.route('/setCliente').post(asyncFunction(async (req, res, next) => {
	...
	if (response.ok) {
		let [datos] = await pool.promise().query('INSERT INTO clientes SET ? ', req.body);
		res.status(200).json({
			ok: true,
...
router.route('/getClientes').get(asyncFunction(async (req, res, next) => {
	...
	if (response.ok) {
		let [datos] = await pool.promise().query('SELECT * FROM clientes WHERE borrado = 0');
		res.status(200).json({
			ok: true,
...
router.route('/deleteCliente').post(asyncFunction(async (req, res, next) => {
	...
	if (response.ok) {
		let [datos] = await pool.promise().query('DELETE  FROM clientes WHERE id = ?', req.body.id); res.status(200).json({
			ok: true,
...
router.route('/updateCliente').post(asyncFunction(async (req, res, next) => {
	...
	if (response.ok) {
		let [datos] = await pool.promise().query('UPDATE  clientes set ? where id = ?', [req.body, req.body.id]);
		res.status(200).json({
			ok: true,
